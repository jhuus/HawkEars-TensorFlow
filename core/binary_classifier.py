...
# noise or low frequency bird sounds (or other sounds that we don't want to treat as noise).
...
class BinaryClassifier:
    def __init__(self, path_prefix):
        self.model = keras.models.load_model(f'{path_prefix}{constants.BINARY_CKPT_PATH}', compile=False)
    ...
    def check_for_noise(self, specs, low_idx, high_idx, low_mult, min_confidence=0.7):
        ...
        while len(indexes) > self.specs.shape[0]:
            ...
            self.specs = np.zeros((old_len + LENGTH_INCR, constants.BINARY_SPEC_HEIGHT, constants.SPEC_WIDTH, 1))
        
        for i in range(len(indexes)):
            self.specs[i, :, :, 0] = specs[indexes[i]][:constants.BINARY_SPEC_HEIGHT,:]
